...
public class AnnotationConfigApplicationContext implements ConfigurableApplicationContext{
     ...
     *
     * 所以，判断是否存在@Component，不但要在当前类查找@Component，还要在当前类的所有注解上，查找该注解是否有@Component，
     * 因此，我们编写了一个能递归查找注解的方法：ClassUtils
     */
...
//            injectProperties(def, def.getBeanClass(), def.getInstance());
     ...
     *
     * 如果我们按照名字查找Bean或BeanDefinition，要么拿到唯一实例，要么不存在，即通过查询Map<String, BeanDefinition>即可完成：
     */
    @Nullable
